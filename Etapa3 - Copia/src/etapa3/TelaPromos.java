...
public class TelaPromos extends javax.swing.JFrame {
    ...
    for (Promocao p : Dados.listaPromocoes) {
        ...
        for (Produto prod : Dados.listaProdutos) {
    ...
    linhaSelecionada = tblPromo.getSelectedRow();
    ...
    Promocao p = Dados.listaPromocoes.get(linhaSelecionada);

    String novaPorcentagem = JOptionPane.showInputDialog("Nova porcentagem:", p.getPorcentagemDesc());
    String novaValidade = JOptionPane.showInputDialog("Nova validade:", p.getDataValidade());

    if (novaPorcentagem != null && novaValidade != null) {
    try {
    p.setPorcentagemDesc(Double.parseDouble(novaPorcentagem));
    p.setDataValidade(novaValidade);
    }   
    catch (Exception e) {
    ...
    if (linhaSelecionada == -1) {
        JOptionPane.showMessageDialog(null, "Nenhuma edição realizada!");
        return;
    }

    carregarTabelaPromocoes();

    JOptionPane.showMessageDialog(null, "Promoção atualizada!");
